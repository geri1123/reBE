...
// Multer instances for different image types (updated for Firebase)
const profileImageUpload = multer({
  storage: memoryStorage, // Changed from createImageStorage('profile_images')
  fileFilter: imageFilter,
  limits: { fileSize: 5 * 1024 * 1024 }, // 5MB
});

const agencyLogoUpload = multer({
  storage: memoryStorage, // Changed from createImageStorage('agency_logo')
  fileFilter: imageFilter,
  limits: { fileSize: 5 * 1024 * 1024 },
});

const generalImageUpload = multer({
  storage: memoryStorage, // Changed from createImageStorage('general')
  fileFilter: imageFilter,
  limits: { fileSize: 5 * 1024 * 1024 },
});

const productImageUpload = multer({
  storage: memoryStorage, // New for product images
  fileFilter: imageFilter,
  limits: { fileSize: 5 * 1024 * 1024 },
});

const documentUpload = multer({
  storage: memoryStorage, // Changed from documentStorage
...
// Export middleware for product images
export const uploadSingleProductImage = productImageUpload.single('image');
export const uploadMultipleProductImages = productImageUpload.array('images', 10);
