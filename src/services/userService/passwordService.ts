...
export class PasswordService extends BaseUserService {
  ...
  private async getStoredPassword(userId: number, language: SupportedLang): Promise<string> {
  ...
  public async changePassword(userId: number, currentPassword: string, newPassword: string, language: SupportedLang): Promise<void> {
    const storedPassword = await this.getStoredPassword(userId, language);
